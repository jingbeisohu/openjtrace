...
* **多协议黑盒**：一个业务请求往往穿透了 `HTTP (Spring Controller) -> RPC (Dubbo) -> MQ (RocketMQ/Kafka) -> SQL (MyBatis XML)`。当表结构或底层 SQL 发生变动时，测试和 Code Review 很难精准识别受�## 🚀 当前可运行能力

`OpenJTrace` 绝非空中楼阁，它率先支持了 Java 企业级最核心的六大关联链路，具备**开箱即用**的静态扫描与分析能力：
...
5. **MQ 异步链路解析 (`analyzer-mq`)**：支持提取并关联 RocketMQ / Kafka / RabbitMQ 的消息发送端与 `@RocketMQMessageListener` / `@RabbitListener` 等消费端，打通异步生产-消费链路。
6. **Redis 缓存依赖分析 (`analyzer-redis`)**：解析 Spring Cache 注解（如 `@Cacheable`、`@CacheEvict` 等）提取缓存分区名；扫描方法体中 `redisTemplate` 的 Key 操作前缀，将方法与 Redis 缓存节点进行关联绑定。
7. **MongoDB 数据库依赖分析 (`analyzer-mongodb`)**：扫描 `MongoRepository` 的继承关系、泛型实体，提取实体类上的 `@Document(collection = "...")` 注解以获得 Collection 名；扫描 `mongoTemplate` 调用的 `.class` 类型，完成 Collection 维度的依赖映射。

通过这套组合拳，`OpenJTrace` 能够完成如下多种链路的逆向追溯：
* **传统 SQL / RPC 影响链**：
  ```text
  [修改 MyBatis XML SQL] 
        │ (MyBatisAnalyzer)
...
* **NoSQL / 缓存 / 异步影响链**：
  ```text
  [修改 MongoDB 实体 @Document / 变更 Redis 缓存 Key / 变更 MQ 消费逻辑] 
        │ (MongoAnalyzer / RedisAnalyzer / MqAnalyzer)
        ▼
  [MongoDB 集合 / Redis 缓存 / MQ 主题] 
        ▲
        │ (CALLS / MAPS_TO)
  [Java 接口与服务方法] 
        │ (HttpAnalyzer)
...
### Month 2: 主流 NoSQL/MQ 深度增强与代码 Diff 自动识别 (8月)
...
* **Redis/Mongo/MQ 复杂操作提取**：支持更多 NoSQL 客户端的静态推导及复杂队列参数绑定。
  ▼
[Java 内部服务/业务逻辑方法] 
      │ (DubboAnalyzer)
      ▼
